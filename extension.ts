...
export function activate(context: vscode.ExtensionContext) {
    ...
    const cursorMoveDisposable = vscode.window.onDidChangeTextEditorSelection(event => {
        ...
        if (hoverInfo[word]) {
            ...
            // Crea un hover para mostrar la información
            const hover = new vscode.Hover(markdownString);

            // Usa el API para mostrar el hover manualmente
            vscode.commands.executeCommand('editor.action.showHover', hover);
        }
    });
